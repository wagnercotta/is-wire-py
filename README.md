...
pytest tests/unit --cov=is_wire --cov-report=html
...
The project uses flake8 for linting and yapf for formatting:
...
is-wire-py/
├── src/is_wire/
│   ├── core/           # Core messaging primitives (Channel, Message, Subscription)
│   ├── rpc/            # RPC support (ServiceProvider, Interceptors)
│   └── enhancement/    # High-level API (IsWireEnhanced)
├── tests/
│   ├── unit/           # Unit tests (no external dependencies)
│   └── conftest.py     # Pytest configuration
├── setup.py            # Package configuration
├── setup.cfg           # Tool configuration (flake8, yapf, tox)
